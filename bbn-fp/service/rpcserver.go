...
// CreateFinalityProvider generates a bbn-fp object and saves it in the database
...
func (r *rpcServer) AddFinalitySignature(_ context.Context, req *proto.AddFinalitySignatureRequest) (
	*proto.AddFinalitySignatureResponse,
	...
	default:
		...
		// if privKey is not empty, then this BTC bbn-fp
		...
		if privKey != nil {
			...
			default:
				msg := fmt.Sprintf(
					"the bbn-fp's BTC private key is extracted but does not match the local key,"+
...
// UnjailFinalityProvider unjails a bbn-fp
func (r *rpcServer) UnjailFinalityProvider(_ context.Context, req *proto.UnjailFinalityProviderRequest) (
	*proto.UnjailFinalityProviderResponse, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to unjail the bbn-fp: %w", err)
	}
...
// QueryFinalityProvider queries the information of the bbn-fp
